...
> = {
  English: {
    "imp-distributed-traffic":
      "A distributed system is almost identical to urban rush hour. In the beginning, when demand is low, every packet travels smoothly through its route. But as concurrent requests spike, single-lane roads—like a central database—become massive bottlenecks. Adding more servers is like adding highway lanes, but if the exit ramp isn't wide enough, traffic just queues up somewhere else. So um... yeah, like when traffic jams happen, you know... cars just stop, and in a system, packets just wait, and then, uh, the queue gets longer and longer and everything is just waiting and waiting because there are too many cars on the same road.",
    default:
      "When we look at this situation, the initial premise is very clear. In the early stage, momentum and clarity guide the direction. However, as complexity mounts, you begin to see unexpected friction points. And so, um, the basic problem is that... you know, things start repeating, and like, it becomes hard to keep going without repeating the same point again and again, basically because the core idea was already stated.",
  },
  Malayalam: {
    "imp-distributed-traffic":
